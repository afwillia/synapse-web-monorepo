...
describe('calculateDefaultColumnWidth', () => {
  describe('Basic Width Calculation', () => {
    ...
    it('should calculate width based on character length (10px per character)', () => {
      // 20 characters * 10px = 200px
      const width = calculateDefaultColumnWidth('this_is_a_long_column')
      expect(width).toBe(210) // 21 characters * 10px = 210px
    })

    it('should use character-based width when it exceeds minimum', () => {
      // 25 characters * 10px = 250px (exceeds 175px minimum)
      const columnName = 'a_very_long_column_name_here'
      const width = calculateDefaultColumnWidth(columnName)
      expect(width).toBe(280) // 28 characters * 10px
    })
  })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle column names with special characters', () => {
      ...
      expect(width).toBe(175) // 17 characters * 10px = 170px, but minimum is 175px
    })
