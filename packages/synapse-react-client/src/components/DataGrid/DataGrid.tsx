...
export default function DataGrid(props: DataGridProps) {
  ...
  // Add a ref to track the animation frame across effect runs
  ...
  useEffect(() => {
    ...
    const restoreScrollAndActiveCell = () => {
      // Check if component unmounted before animation frame completed
      if (!isMounted) {
        return
      }
      ...
      if (!gridContainer && retryCount < MAX_RETRIES) {
        retryCount++
        animationFrameIdRef.current = requestAnimationFrame(
          restoreScrollAndActiveCell,
        )
        return
      }
    ...
    // Cancel any pending animation frame from previous effect run
