...
export function useColumnResizeHandles({
  ...
  useEffect(() => {
    // Create new manager with current props
    ...
    // Use ResizeObserver to detect when wrapper has been sized/rendered
    const initializeWhenReady = () => {
      ...
      resizeObserver = new ResizeObserver(entries => {
        ...
        // Check if the wrapper has non-zero dimensions (indicating it's rendered)
        const entry = entries[0]
        if (entry && entry.contentRect.height > 0) {
          // Attempt to initialize the manager
          const success = manager.initialize(wrapperRef.current)

          // If successful, disconnect the observer as we're done
          if (success && resizeObserver) {
    ...
    return () => {
      // Mark component as unmounted to prevent callbacks from executing
      isMounted = false
      if (resizeObserver) {
        resizeObserver.disconnect()
      }
  ...
  }, [colValues, wrapperRef, onColumnResize])
}
